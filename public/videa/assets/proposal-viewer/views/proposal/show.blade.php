    ...
    <div class="col-lg-3">
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            ...
            {{  $xml->Proposal->Seller->Salesperson->Phone[1]['type'] }}: {{  $xml->Proposal->Seller->Salesperson->Phone[1] }}<br>
            </p>
          </div>
        </div>
    </div>
        ...
        @foreach( $xml->Proposal->AvailList->AvailLineWithDetailedPeriods as $avails )
            @foreach($avails->Periods->DetailedPeriod as $period)
                <tr>
                    ...
                    <td class="rate text-right currency">{{ number_format((float)$period->Rate, 2, ".", ",") }}</td>
                    <td class="text-center">{{ $period->DemoValues->DemoValue }}</td>
                    @if( (float)$period->DemoValues->DemoValue > 0 )
                      <td class="cpp text-right currency">{{ number_format( (float)($period->Rate)/(float) $period->DemoValues->DemoValue,2, ".", ",")  }}</td>
                    @else
                    ...
                    <td class="text-center">{{$avails->DayTimes->DayTime->EndTime }}
