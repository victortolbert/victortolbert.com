...
export default {
  ...
  computed: {
    ...
    floatOffset() {
      return Number.parseFloat(this.slideOffset)
    },
    floatKeyboardStep() {
      return Number.parseFloat(this.keyboardStep)
    },
  },
  mounted() {
    ...
    moveSlide(event) {
      ...
      if (event.key) {
        switch (event.key) {
          ...
          case 'ArrowLeft': this.slideOffset = ((this.floatOffset - this.floatKeyboardStep) >= 0)
            ? this.floatOffset - this.floatKeyboardStep
            : 0
            break
          case 'Right': // IE/Edge key
          case 'ArrowRight': this.slideOffset = ((this.floatOffset + this.floatKeyboardStep) <= 1)
            ? this.floatOffset + this.floatKeyboardStep
            : 1
            break
          default:
