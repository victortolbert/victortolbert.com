...
<template>
  <UContainer v-if="recipe">
    ...
    <div class="flex flex-row gap-1 mt-2">
      <UBadge v-for="label in recipe.dishTypes" :key="`recipe-${label}`" :label="label" variant="subtle" />
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-2 md:gap-8">
      ...
      <section class="py-2 md:py-8">
        ...
        <ol class="flex flex-col gap-1 md:gap-4 mt-5 w-full max-w-[60ch]">
          <li v-for="{ number, step } in recipe.analyzedInstructions?.[0].steps" :key="`step-${number}-${step}`" class="flex items-baseline gap-2">
