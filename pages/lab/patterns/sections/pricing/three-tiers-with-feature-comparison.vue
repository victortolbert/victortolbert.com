    ...
    <div class="flow-root bg-gray-900 pb-16 pt-24 sm:pt-32 lg:pb-0">
      <div class="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div class="mt-16 flex justify-center">
            <RadioGroup v-model="frequency" class="grid grid-cols-2 gap-x-1 rounded-full bg-white/5 p-1 text-center text-xs font-semibold leading-5 text-white">
              ...
              <RadioGroupOption v-for="option in frequencies" :key="option.value" v-slot="{ checked }" as="template" :value="option">
                <div class="cursor-pointer rounded-full px-2.5 py-1" :class="[checked ? 'bg-primary-500' : '']">
        ...
        <div class="relative mx-auto mt-10 grid max-w-md grid-cols-1 gap-y-8 lg:mx-0 lg:-mb-14 lg:max-w-none lg:grid-cols-3">
          ...
          <div v-for="tier in tiers" :key="tier.id" class="relative rounded-2xl" :class="[tier.featured ? 'z-10 bg-white shadow-xl ring-1 ring-gray-900/10' : 'bg-gray-800/80 ring-1 ring-white/10 lg:bg-transparent lg:pb-14 lg:ring-0']">
            <div class="p-8 lg:pt-12 xl:p-10 xl:pt-14">
              <h3 :id="tier.id" class="text-sm font-semibold leading-6" :class="[tier.featured ? 'text-gray-900' : 'text-white']">
                {{ tier.name }}
              </h3>
              <div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between lg:flex-col lg:items-stretch">
                <div class="mt-2 flex items-center gap-x-4">
                  <p class="text-4xl font-bold tracking-tight" :class="[tier.featured ? 'text-gray-900' : 'text-white']">
                ...
                <a :href="tier.href" :aria-describedby="tier.id" class="rounded-md py-2 px-3 text-center text-sm font-semibold leading-6 text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" :class="[tier.featured ? 'bg-primary-600 shadow-sm hover:bg-primary-500 focus-visible:outline-primary-600' : 'bg-white/10 hover:bg-white/20 focus-visible:outline-white']">Buy this plan</a>
              </div>
              <div class="mt-8 flow-root sm:mt-10">
                <ul role="list" class="-my-2 divide-y border-t text-sm leading-6 lg:border-t-0" :class="[tier.featured ? 'divide-gray-900/5 border-gray-900/5 text-gray-600' : 'divide-white/5 border-white/5 text-white']">
                  <li v-for="mainFeature in tier.mainFeatures" :key="mainFeature" class="flex gap-x-3 py-2">
                    <CheckIcon class="h-6 w-5 flex-none" :class="[tier.featured ? 'text-primary-600' : 'text-gray-500']" aria-hidden="true" />
                    {{ mainFeature }}
                  </li>
                </ul>
              </div>
        ...
        <section aria-labelledby="mobile-comparison-heading" class="lg:hidden">
          ...
          <div class="mx-auto max-w-2xl space-y-16">
            <div v-for="tier in tiers" :key="tier.id" class="border-t border-gray-900/10">
              <div class="-mt-px w-72 border-t-2 pt-10 md:w-80" :class="[tier.featured ? 'border-primary-600' : 'border-transparent']">
                <h3 class="text-sm font-semibold leading-6" :class="[tier.featured ? 'text-primary-600' : 'text-gray-900']">
                  {{ tier.name }}
                </h3>
              ...
              <div class="mt-10 space-y-10">
                <div v-for="section in sections" :key="section.name">
                  ...
                  <div class="relative mt-6">
                    ...
                    <div class="relative rounded-lg bg-white shadow-sm sm:rounded-none sm:bg-transparent sm:shadow-none sm:ring-0" :class="[tier.featured ? 'ring-2 ring-primary-600' : 'ring-1 ring-gray-900/10']">
                      <dl class="divide-y divide-gray-200 text-sm leading-6">
                        <div v-for="feature in section.features" :key="feature.name" class="flex items-center justify-between px-4 py-3 sm:grid sm:grid-cols-2 sm:px-0">
                          ...
                          <dd class="flex items-center justify-end sm:justify-center sm:px-4">
                            <span v-if="typeof feature.tiers[tier.name] === 'string'" :class="tier.featured ? 'font-semibold text-primary-600' : 'text-gray-900'">{{ feature.tiers[tier.name] }}</span>
                            <template v-else>
                              <CheckIcon v-if="feature.tiers[tier.name] === true" class="mx-auto h-5 w-5 text-primary-600" aria-hidden="true" />
                    ...
                    <!-- Fake card border -->
                    <div aria-hidden="true" class="pointer-events-none absolute inset-y-0 right-0 hidden w-1/2 rounded-lg sm:block" :class="[tier.featured ? 'ring-2 ring-primary-600' : 'ring-1 ring-gray-900/10']" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <section aria-labelledby="comparison-heading" class="hidden lg:block">
          ...
          <div class="grid grid-cols-4 gap-x-8 border-t border-gray-900/10 before:block">
            <div v-for="tier in tiers" :key="tier.id" aria-hidden="true" class="-mt-px">
              <div class="border-t-2 pt-10" :class="[tier.featured ? 'border-primary-600' : 'border-transparent']">
                <p class="text-sm font-semibold leading-6" :class="[tier.featured ? 'text-primary-600' : 'text-gray-900']">
                  {{ tier.name }}
                </p>
                <p class="mt-1 text-sm leading-6 text-gray-600">
                  {{ tier.description }}
            ...
            <div v-for="section in sections" :key="section.name">
              ...
              <div class="relative -mx-8 mt-10">
                    ...
                    <tr v-for="(feature, featureIdx) in section.features" :key="feature.name">
                      ...
                      <td v-for="tier in tiers" :key="tier.id" class="relative w-1/4 px-4 py-0 text-center">
                        <span class="relative h-full w-full py-3">
                          <span v-if="typeof feature.tiers[tier.name] === 'string'" class="text-sm leading-6" :class="[tier.featured ? 'font-semibold text-primary-600' : 'text-gray-900']">{{ feature.tiers[tier.name] }}</span>
                          <template v-else>
                            <CheckIcon v-if="feature.tiers[tier.name] === true" class="mx-auto h-5 w-5 text-primary-600" aria-hidden="true" />
                            <XMarkIcon v-else class="mx-auto h-5 w-5 text-gray-400" aria-hidden="true" />
                            <span class="sr-only">{{ feature.tiers[tier.name] === true ? 'Yes' : 'No' }}</span>
                ...
                <div class="pointer-events-none absolute inset-x-8 inset-y-0 grid grid-cols-4 gap-x-8 before:block" aria-hidden="true">
                  <div v-for="tier in tiers" :key="tier.id" class="rounded-lg" :class="[tier.featured ? 'ring-2 ring-primary-600' : 'ring-1 ring-gray-900/10']" />
                </div>
              </div>
            </div>
          </div>
