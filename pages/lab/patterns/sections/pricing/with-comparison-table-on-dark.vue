  ...
  <div class="bg-gray-900 py-24 sm:py-32">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      ...
      <div class="mx-auto mt-12 max-w-md space-y-8 sm:mt-16 lg:hidden">
        <section v-for="tier in tiers" :key="tier.id" class="p-8" :class="[tier.mostPopular ? 'rounded-xl bg-white/5 ring-1 ring-inset ring-white/10' : '']">
          <h3 :id="tier.id" class="text-sm font-semibold leading-6 text-white">
            {{ tier.name }}
          </h3>
          ...
          <a :href="tier.href" :aria-describedby="tier.id" class="mt-8 block rounded-md py-2 px-3 text-center text-sm font-semibold leading-6 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" :class="[tier.mostPopular ? 'bg-primary-500 text-white hover:bg-primary-400 focus-visible:outline-primary-500' : 'bg-white/10 text-white hover:bg-white/20 focus-visible:outline-white']">Buy plan</a>
              ...
              <ul role="list" class="space-y-4">
                <template v-for="feature in section.features" :key="feature.name">
                  <li v-if="feature.tiers[tier.name]" class="flex gap-x-3">
                    <CheckIcon class="h-6 w-5 flex-none text-primary-400" aria-hidden="true" />
                    <span>
                      {{ feature.name }}
                      {{ ' ' }}
            ...
            <tbody>
              <tr>
                ...
                <td v-for="tier in tiers" :key="tier.id" class="px-6 pt-2 xl:px-8">
                  ...
                  <a :href="tier.href" class="mt-8 block rounded-md py-2 px-3 text-center text-sm font-semibold leading-6 text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" :class="[tier.mostPopular ? 'bg-primary-500 hover:bg-primary-400 focus-visible:outline-primary-600' : 'bg-white/10 hover:bg-white/20 focus-visible:outline-white']">Buy plan</a>
                </td>
              </tr>
              <template v-for="(section, sectionIdx) in sections" :key="section.name">
                <tr>
                  <th scope="colgroup" colspan="4" class="pb-4 text-sm font-semibold leading-6 text-white" :class="[sectionIdx === 0 ? 'pt-8' : 'pt-16']">
                    {{ section.name }}
                ...
                <tr v-for="feature in section.features" :key="feature.name">
                  ...
                  <td v-for="tier in tiers" :key="tier.id" class="px-6 py-4 xl:px-8">
                    <div v-if="typeof feature.tiers[tier.name] === 'string'" class="text-center text-sm leading-6 text-gray-300">
                      {{ feature.tiers[tier.name] }}
                    </div>
                    <template v-else>
                      <CheckIcon v-if="feature.tiers[tier.name] === true" class="mx-auto h-5 w-5 text-primary-400" aria-hidden="true" />
